...
Before marking chapter 9 complete, `tests/run_all_tests.ps1` SHALL exit zero with all unit test files passing.

#### Scenario: All unit tests green

- **WHEN** `tests/run_all_tests.ps1` is executed on the release candidate branch
- **THEN** output includes `ALL N TESTS PASSED` with exit code 0
